...
public class LinkGeoTweets {
	...
	public static void main(String[] args)
	{
		...
		if (!(args.length == 1)) {
			System.err.println("PLease enter the search term");
	        System.exit(1);
		}
		else
		{
		...
		try
		{
			...
			for (int queryNumber=0;queryNumber < MAX_QUERIES; queryNumber++)
			{
				...
				Query q = new Query(searchTerm);			// Search for tweets that contains this term
				...
				for (Status s: r.getTweets())				// Loop through all the tweets...
				{
					...
					try (PrintWriter out = new PrintWriter("Output_Geotwitter.txt")) {
						out.printf("At %s, @%-20s said:  %s\n",
